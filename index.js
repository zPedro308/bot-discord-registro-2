...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isChatInputCommand()) {
    if (interaction.commandName === 'enviarpainelregistro') {
      const embed = new EmbedBuilder()
        .setTitle('Sistema de Registro - Guarulhos')
        .setDescription(
          '👮‍♂️ Bem-vindo ao sistema de registro da **Guarulhos**!\n\n' +
      ...
      const canal = await client.channels.fetch('1381345750188822648');
  ...
  if (interaction.isButton()) {
    ...
    if (!interaction.member.roles.cache.has('1313114123386622017')) {
    ...
    const canalLogs = await client.channels.fetch('1381346489200148713');

    if (action === 'aceitar' && membro) {
      const nick = ` ${nome}「${id}」`.slice(0, 32);
      await membro.setNickname(nick).catch(() => null);

      const cargos = ['1313114123344674834', 'tag', 'tag', 'tag', 'tag'];
